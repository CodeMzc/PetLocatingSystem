...
void GPS_Data(void)
{
		float tp;
		tp = gpsx.longitude;
		sprintf((char *)jd,"Longitude:%.5f%1c",tp/=100000,gpsx.ewhemi);	//得到经度字符串
		tp = gpsx.latitude;
		sprintf((char *)wd,"Latitude:%.5f%1c",tp/=100000,gpsx.nshemi);	//得到纬度字符串
//		UART1_SendString("\r\n提取出的数据：\r\n");
//		UART1_SendString("\r\n 经度：\n");
...
void USART1_IRQHandler(void)                	
{
			...
			Uart1_Buf[Second_Int] = Res;  	  //将接收到的字符串存到缓存中
			Second_Int++;                			//缓存指针向后移动
			if(Second_Int > Buf1_Max)       		//如果缓存满,将缓存指针指向缓存的首地址
			{
				Second_Int = 0;
			} 
} 	
...
void Connect_Server(void)
{
	...
	Second_AT_Command("AT+CIPSHUT","SHUT OK",3);		//关闭移动场景
	Second_AT_Command("AT+CGCLASS=\"B\"","OK",3);//设置GPRS移动台类别为B,支持包交换和数据交换 
	Second_AT_Command("AT+CGDCONT=1,\"IP\",\"CMNET\"","OK",3);//设置PDP上下文,互联网接协议,接入点等信息
	Second_AT_Command("AT+CGATT=1","OK",4);//附着GPRS业务
	Second_AT_Command("AT+CIPCSGP=1,\"CMNET\"","OK",3);//设置为GPRS连接模式
	Second_AT_Command("AT+CIPHEAD=1","OK",3);//设置接收数据显示IP头(方便判断数据来源,仅在单路连接有效)
